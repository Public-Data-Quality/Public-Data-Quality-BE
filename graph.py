...
def build_graph(llm_model: str | None = None):
    agents = build_agents(llm_model=llm_model)
    ...
    graph.add_node("profile_values", agents["profiler"].run)
    ...
    graph.add_node("propose_repairs", agents["repair_planner"].run)
    graph.add_node("verify_results", agents["verifier"].run)
    ...
    graph.add_conditional_edges("route_rules", should_run_rag, {"rag_resolve": "rag_resolve", "validate": "semantic_profile"})
